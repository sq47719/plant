...
void WiFi_ResetIO_Init(void)
{
	...
	RCC_APB2PeriphClockCmd( RCC_APB2Periph_ALi_GPIO_Port , ENABLE); //使能PA端口时钟
	
	GPIO_InitStructure.GPIO_Pin = ALi_GPIO_Pin_Num_Rst;               //准备设置端口号
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;                 //速率50Mhz
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;   	              //推免输出方式
	GPIO_Init(ALi_GPIO_Port, &GPIO_InitStructure);            	      //设置输出模式
	RESET_IO(1);                                                      //复位IO拉高电平
}

/*************************************************
阿里云  ESP8266  GPIO以及通信接口的初始化
*************************************************/
void ALi_ESP8266_GPIO_Init()
{
	Usart2_Init(115200);
	WiFi_ResetIO_Init();
