...
//void Get_tem_hum_value(u8 *tem,u8 *hum);
/*----------------------------------------------------------*/
...
//以下数据的发送形参个数   根据发送个数进行修改
void send_data(u8 *tem,u8 *hum,int *turann,int *cooo,int *guangg,int *aa,int *bb,int *cc,int *dd,int *nn)/*****************************************************数据发送******************************************************************/
{			
	char temt[256];				//定义一个临时缓冲区1,不包括报头
	int temperature = *tem;
	int humilty = *hum;
    u16 turan = *turann;
	int coo = *cooo;
  ...
  int c = *cc;
	...
	sprintf(temt,"{\"method\":\"thing.event.property.post\",\"id\":\"203302322\",\"params\":{\"temperature\":%d,\"humidity\":%d,\"turan\":%d,\"coo\":%d,\"guang\":%d,\"a\":%d,\"b\":%d,\"c\":%d,\"d\":%d,\"n\":%d},\"version\":\"1.0.0\"}",temperature,humilty,turan,coo,guang,a,b,c,d,n);  //构建回复湿度温度数据
	MQTT_PublishQs0(P_TOPIC_NAME,temt,strlen(temt));
