...
#define RCC_APB2Periph_RC522_GPIO_Port_RST      RCC_APB2Periph_GPIOB    //RC522用到的SPI是控制器内部的SPI   此处的初始化是对复位等引脚初始化  SPI相关引脚初始化在SPI.c文件中
#define RC522_GPIO_Pin_Num_RST                  GPIO_Pin_9
#define RC522_GPIO_Pin_Num_CS                   GPIO_Pin_12
#define RC522_GPIO_Port                         GPIOB

#define CS_0     GPIO_ResetBits(GPIOB,GPIO_Pin_12)
#define CS_1     GPIO_SetBits(GPIOB,GPIO_Pin_12)
#define RST_0    GPIO_ResetBits(GPIOB,GPIO_Pin_9)
#define RST_1    GPIO_SetBits(GPIOB,GPIO_Pin_9)
...
void RC522_GPIO_Init_STM32_IN_Spi(void);         //RFID_RC522 GPIO SPI初始化  完成对RC522全部引脚包括控制器内部SPI的初始化    
void RC522_Care_Incre_Handle(_Bool *Incre_fla);  //在处理函数中完成对卡片状态读取的函数
void RC522_Care_Dre_Handle(_Bool *Dre_fla);
void Get_RC522_Care_ID(u8 *RC522_Care_I);        //用于读取卡片的ID
