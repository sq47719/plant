...
这个函数通过设定卡片ID数组，实现对是否刷卡的判断
************************************************/

/*************************************************

_Bool Care_Incre_Flag;         
_Bool Care_Dre_Flag;
这两个标志变量是用来判断卡片是否刷下的标志位

主函数完成对卡片是否刷下的判断是通过下面两个函数实现的
void RC522_Care_Incre_Handle(_Bool *Incre_fla);
void RC522_Care_Dre_Handle(_Bool *Dre_fla);

**************************************************/
static _Bool RC522_Care_Incre_Flag;         
static _Bool RC522_Care_Dre_Flag;
u8 RC522_Care_ID[4];  //读取卡片ID

unsigned short Card_Type;                          //卡的类型
unsigned char Card_ID[4];                          //用于存放卡的序列号
unsigned char Card_Buffer[2];

void RC522_GPIO_Init_STM32_IN_Spi(void)
{
	GPIO_InitTypeDef GPIO_InitStructure;
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_RC522_GPIO_Port_RST,ENABLE);
    
	GPIO_InitStructure.GPIO_Pin=RC522_GPIO_Pin_Num_CS;                   //RFID_CS
	GPIO_InitStructure.GPIO_Mode=GPIO_Mode_Out_PP;                       //推挽输出
	GPIO_InitStructure.GPIO_Speed=GPIO_Speed_50MHz;
	GPIO_Init(RC522_GPIO_Port,&GPIO_InitStructure);
	GPIO_SetBits(RC522_GPIO_Port,RC522_GPIO_Pin_Num_CS);                 //拉高RFID_CS
//------------------------------------------------------------------ 
	
	GPIO_InitStructure.GPIO_Pin=RC522_GPIO_Pin_Num_RST;                  //RFID_RST
	GPIO_InitStructure.GPIO_Mode=GPIO_Mode_Out_PP;                       //推挽输出
	GPIO_InitStructure.GPIO_Speed=GPIO_Speed_50MHz;
	GPIO_Init(RC522_GPIO_Port,&GPIO_InitStructure);
	GPIO_SetBits(RC522_GPIO_Port,RC522_GPIO_Pin_Num_RST);                //拉高
		...
		if (status == MI_OK)
    {
         ...
         {   
            *(pSnr+i)  = ucComMF522Buf[i];//将卡片的序列号存入数组
					 RC522_Care_ID[i]=ucComMF522Buf[i];
...
//	unsigned char Block=4;
	unsigned char status;
 	...
 	if(PcdRequest(request_mode,Card_Buffer)==MI_OK)    //寻卡 寻天线内未进入休眠的卡
	{
		card_type=(card_buffer[0]<<8)|card_buffer[1];
		if(card_type==0x0400)                            //判断为M1_S50卡
		{
//			printf("Request_OK_M1_S50\r\n");  //常
			if(PcdAnticoll(card_id)==MI_OK)                //防冲撞   返回卡4个字节的序列号
			{
//				printf("Card_ID:"); //串口打印
//				Printing(Card_ID,4);
...
void RC522_Care_Incre_Handle(_Bool *Incre_fla)
{
	...
	{
			if(RC522_Care_Incre_Flag==1)  //卡片被刷后  标志位置1
		    *Incre_fla=1;
	    else
		   *Incre_fla=0;
	}
...
void RC522_Care_Dre_Handle(_Bool *Dre_fla)
{
	...
	if(Request_Anticoll_Select(PICC_REQALL,Card_Type,Card_Buffer,Card_ID)==MI_OK)
	{
			if(RC522_Care_Dre_Flag==1)  //卡片被刷后  标志位置1
		     *Dre_fla=1;
	    else
		    *Dre_fla=0;
	}
...
void Get_RC522_Care_ID(u8 *RC522_Care_I)
{
	u8 i;	
	...
	{
			for(i=0;i<4;i++)
	    *(RC522_Care_I+i)= RC522_Care_ID[i];
	}
}
